...
Your task is to construct a knowledge graph from the provided database schema. This graph must include only the elements and relationships explicitly described below. Do not create any additional nodes or relationships that are not part of this structure.
...
Now, based on the provided input database schema, extract the elements and construct the graph in the required format.
""")

DB_GRAPH_STRUCTURE_TIP = HumanMessagePromptTemplate(
    prompt=PromptTemplate.from_template("""
Carefully extract information from the database schema and strictly adhere to the specified graph structure.
Remember:
- Only use the node types: TABLE, COLUMN.
- Only use the relationship types: HAS_COLUMN, FOREIGN_KEY_TO, RELATES_TO.
- Do not group unrelated information into generic nodes like "Document."
